"""
EI Report Generator - Server Module
====================================
Refactored from ei_report_generator.py for API use.
Accepts a source XLSX path, returns the generated report as bytes.
"""
...
def parse_raw_tab(ws):
    ...
    dc_idx      = col_map.get('source_dc')
    track_idx   = col_map.get('final_tracking_no')
    fwd_agt_idx = col_map.get('fwd_agent name')
    rev_agt_idx = col_map.get('rev_agent name')

    if dc_idx is None:
        raise ValueError('Column Source_DC not found in Raw tab.')
    if track_idx is None:
        raise ValueError('Column Final_tracking_no not found in Raw tab.')
...
class ReportError(Exception):
    """Raised when report generation fails due to bad input data."""
    pass


def generate_report(source_path: str) -> bytes:
    """
    Generate the EI Summary report from a source XLSX file.

    Args:
        source_path: Path to the source E2E Task XLSX file.

    Returns:
        The generated EI_SUMMARY report as XLSX bytes.

    Raises:
        ReportError: If the source file is invalid or missing required sheets.
    """
    if not os.path.exists(source_path):
        raise ReportError(f"Source file not found: {source_path}")

    wb_src = openpyxl.load_workbook(source_path, data_only=True)

    # ── Task_per_1k -> SUMMARY tab ──
    if 'Task_per_1k' not in wb_src.sheetnames:
        raise ReportError(
            f"Sheet 'Task_per_1k' not found. Available: {wb_src.sheetnames}"
        )
    ...
    # ── Raw -> Filtered / FWD / REV / Agent Summary tabs ──
    if 'Raw' not in wb_src.sheetnames:
        headers, filt_rows, col_map, track_idx, fwd_agt_idx, rev_agt_idx, dc_idx = (
            [], [], {}, None, None, None, None
        )
    else:
        headers, filt_rows, col_map, track_idx, fwd_agt_idx, rev_agt_idx, dc_idx = (
            parse_raw_tab(wb_src['Raw'])
        )
